...
def main():
    ...
    df = pd.read_parquet("data/processed/q3_team_v2.parquet").dropna(subset=["final_total", "final_margin"])
    ...
    # Compute residuals
    resid_t = df["final_total"].values - pred_t
    resid_m = df["final_margin"].values - pred_m
