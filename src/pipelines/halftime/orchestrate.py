...
def run_halftime_pipeline() -> None:
    settings = load_settings()
    pipeline = settings.halftime_pipeline

    if not pipeline.steps:
        raise RuntimeError("No pipeline steps configured for halftime pipeline.")
    ...
    contract_path = settings.project_root / "data/contracts/halftime_team_v2.json"
    dataset_path = settings.project_root / "data/processed/halftime_team_v2.parquet"
