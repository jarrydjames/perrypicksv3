...
print('BACKTESTING PREGAME MODEL WITH ACCURACY METRICS')
print()
...
for i in range(num_folds):
    ...
    acc_3 = np.sum(np.abs(y_test_total - pred_total) <= 3) / len(y_test_total) * 100
    acc_5 = np.sum(np.abs(y_test_total - pred_total) <= 5) / len(y_test_total) * 100
    acc_10 = np.sum(np.abs(y_test_total - pred_total) <= 10) / len(y_test_total) * 100
    
    # ROI calculation (margin only)
    ...
    print(f'Fold {i+1}/{num_folds}: Total MAE={total_mae:.3f}, Acc@3pt={acc_3:.1f}%, ROI={roi:.1f}%')
...
results_df.to_parquet('data/processed/pregame_backtest_results_with_accuracy.parquet', index=False)
...
print()
print('ACCURACY (Within X points):')
print(f'  Within 3 points:  {results_df["acc_3"].mean():.1f}% (±{results_df["acc_3"].std():.1f}%)')
print(f'  Within 5 points:  {results_df["acc_5"].mean():.1f}% (±{results_df["acc_5"].std():.1f}%)')
print(f'  Within 10 points: {results_df["acc_10"].mean():.1f}% (±{results_df["acc_10"].std():.1f}%)')
print()
print(f'  ROI: {results_df["roi"].mean():.1f}% (±{results_df["roi"].std():.1f}%)')
print()
print('PREGAME BACKTEST WITH ACCURACY DONE!')
